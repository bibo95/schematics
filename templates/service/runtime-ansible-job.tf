...
resource "kubernetes_deployment" "runtime_ansible_job" {
  ...
  metadata {
    ...
    annotations = {
      "kubernetes.io/change-cause" = "schematics-ansible-job_{IMAGE_TAG}"
    }
  }
  ...
  depends_on = [kubernetes_service.job_service, kubernetes_config_map.runtime_ansible_job_configmap, kubernetes_namespace.namespace, kubernetes_secret.schematics-job-secret]
}
